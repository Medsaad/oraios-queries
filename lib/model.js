...
class Model extends QueryBuilder {
	...
	/**
	 * query a large amount of data in chunks
	 * @param {number} count 
	 * @param {function} callback 
	 */
	async chunk(count, callback) {
		this._query.iterative = true;

		try {
			let hasMore = true;
			let chunkCount = 0;
			let chunkQuery = this._query;

			while (hasMore) {

				let rows = await this.paginate(count, ++chunkCount, this._query.iterative);

				if (!rows && chunkCount === 1) {
					rows = [];
				}
				...
				// enabling user to work with any query with the callback
				this._query = {};

				await callback(rows);
				
				// reset the chunk query again to be executed

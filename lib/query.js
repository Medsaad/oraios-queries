...
class QueryBuilder {
	...
	select(select) {
		if (!Array.isArray(select)) {
			throw new SyntaxError(`select() parameter 1 should be an array, ${typeof select} given.`);
		}
		...
		for (let index = 0; index < select.length; index++) {
			...
			if (typeof field === 'string') {
				...
				throw new SyntaxError('Unknown data type select field', field);
			}
		};
	...
	_join(type, rTable, leftField, rightField) {
		if (!(rTable instanceof QueryBuilder)) {
			throw new SyntaxError(`join parameter 1 should be an object of a certain model, ${typeof rTable} given.`);
		}

		let rightTable = rTable.tableName;
		if (!rightTable) {
			throw new SyntaxError('Joined model table name not found', rTable);
		}
	...
	orderBy(orderByList) {
		if (!Array.isArray(orderByList)) {
			throw new SyntaxError(`orderBy() parameter 1 should be an array, ${typeof orderByList} given.`);
		}
	...
	groupBy(groupList) {
		if (!Array.isArray(groupList)) {
			throw new SyntaxError(`groupBy() parameter 1 should be an array, ${typeof groupList} given.`);
		}
	...
	set(data) {
		if (typeof data !== 'object') {
			throw new SyntaxError(`set() parameter 1 should be an object, ${typeof data} given.`);
		}

		//sanitizing data
		for (const col in data) {
			if (data[col] === undefined) {
				throw new SyntaxError(`Value for column ${col} can not be undefined.`);
			}
	...
	setMany(data) {
		if (!Array.isArray(data)) {
			throw new SyntaxError(`setMany() parameter 1 should be an array, ${typeof data} given.`);
		}
	...
	/**
	 * returns the query string without execution
	 */
	toString() {
		this._query.stringify = true;
		return this;
	}
}

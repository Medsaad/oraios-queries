...
class WhereParser {
    ...
    _parseConditions(conditions) {
        ...
        if (typeof conditions == 'object') {
            ...
            throw new SyntaxError("Unknown condition type for where: " + typeof conditions);
        }
    }
    ...
    _validateContition(condition) {
        ...
        if (condition.length != 3) {
            throw new SyntaxError('Condition is corrupted. Condition array should have 3 items [column, operand, value].', condition);
        }
        ...
        if (isNaN(condition[2]) && !Array.isArray(condition[2]) && typeof condition[2] !== 'string' && typeof condition[2] !== 'object') {
            throw new SyntaxError("Unknown condition value data type: " + (typeof condition[2]));
        }
        ...
        if (['in', 'not in'].includes(operand) && !Array.isArray(condition[2])) {
            throw new SyntaxError("Condition array value should only passed with an 'in'");
        }

        if (!['in', 'not in'].includes(operand) && Array.isArray(condition[2])) {
            throw new SyntaxError(`Invalid input passed with operand: ${operand}`);
        }
